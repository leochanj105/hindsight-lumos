...
func AgentServerInit(fname string, delay int64) *AgentServer {
    var server AgentServer
    server.report_queue.Req = make(map[int64]int64)
    server.api = memory.InitAgentAPI(fname)
    server.delay = delay
    return &server
}

/*
The QueueServer drains full buffers from the complete queue,
adds them to the cache manager, and pushes evicted buffers from
the cache manager back to the available queue.
*/
func (agent *AgentServer) RunQueueServer() {
    for true {
        complete := agent.GetComplete()
        if len(complete) == 0 {
            continue
        }
